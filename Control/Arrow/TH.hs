...
f (UInfixE expr3 expr mexpr2) = [| UInfixE $(f expr3) $(f expr) $(f mexpr2) |]
f (InfixE (Just expr3) expr (Just expr2)) = [| InfixE (Just $(f expr3)) $(f expr) (Just $(f expr2)) |]
f (InfixE (Just expr3) expr Nothing) = [| InfixE (Just $(f expr3)) $(f expr) Nothing |]
f (InfixE Nothing expr (Just expr2)) = [| InfixE Nothing ($(f expr)) (Just $(f expr2)) |]
f (InfixE Nothing expr Nothing) = [| InfixE Nothing $(f expr) Nothing |]
